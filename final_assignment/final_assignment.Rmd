...
percentile_ci_aus <- get_ci(boot_dist_aus , level = 0.95, type ="percentile") %>% 
  mutate(country_code = "AUS",
         mean = mean_median_tech_salary_by_country[[2,2]]) %>% 
  relocate(country_code) 
...
percentile_ci_gbr <- get_ci(boot_dist_gbr , level = 0.95, type ="percentile") %>% 
  mutate(country_code = "GBR",
         mean = mean( mean_median_tech_salary_by_country[[4,2]])) %>% 
  relocate(country_code)
...
percentile_ci_can <- get_ci(boot_dist_can , level = 0.95, type ="percentile") %>% 
  mutate(country_code = "CAN",
         mean = mean_median_tech_salary_by_country[[5,2]]) %>% 
  relocate(country_code)
...
percentile_ci_median_tech <- data.frame(matrix(ncol = 4,nrow = 0))

colnames(percentile_ci_median_tech)<-c("country","lower_ci", "upper_ci","median")
...
#creating the CI table with country code
percentile_ci <- get_ci(boot_dist , level = 0.95, type ="percentile")

percentile_ci_median_tech <- percentile_ci_median_tech%>%
  mutate(country = i,
         lower_ci = percentile_ci$lower_ci,
         upper_ci = percentile_ci$upper_ci,
