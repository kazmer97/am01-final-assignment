...
single_race_data%>%
  ...
  geom_text(aes(label = sprintf("%.2f%%", num_resp_race/sum(num_resp_race)*100)), hjust = 0.5)
...
ask_a_manager_2021_cleaned%>%
  ...
  labs(title = "Number of Responses By Gender",
       x = "number of responses")+
  theme(axis.title.y = element_blank())+
  geom_text(aes(label = sprintf("%.2f%%", num_resp_gender/sum(num_resp_gender)*100)), hjust = 0.5)
```

## Number of Responses by Country

```{r num-res-per-country, fig.cap = "Number of Resp/Country(ref:foo)"}
number_resposes_country <- ask_a_manager_2021_cleaned%>%
...
ask_a_manager_2021_cleaned%>%
  ...
  labs(title = "Number of Responses By Country (with more than 50 responses)",
       x = "number of responses")+
  theme(axis.title.y = element_blank())+
  geom_text(aes(label = sprintf("%.2f%%", num_resp_country/sum(num_resp_country)*100)), hjust = 0.5)
```
...
The race distribution of the dataset is not representative of the countries population. For example the United States has an African American population of over 13% while in the dataset it below 5%.

## Gender distribution from countries with more then 50 responses
...
From the top countries the responses overwhelmingly are from women, execpt the netherlands where the data is very close to balanced.
...
As the Race is not representative of the countries real population this graph only informs us of the distribution of the survey respondents across industries and it is more revealing of the blogs reader base rather then industry statistics.
...
ggplot(m_race_gender_summary, aes(y = number, x = reorder(n_race, number)))+
  ...
  labs(
    ...
    x = "Number of Race options ticked by Respondant",
...
H0 cannot as p-value is above 0.05. Which means there is no difference in the likelihood of identifying with multiple ethnic bakcgrounds between millenial men and women. 
...
## What is the most popular industry among the top five countries with the most responses?

We would like to see the popularity of the Computing or Tech industry in each country:

```{r gender-industry-country, fig.cap="% of Industry and Gender distribution"}
...
ggplot(salary_exploration, aes(x = prc, y = reorder(industry_cleaned, prc)))+
...
### Confidence interval of Median Salaries
...
### Confidence interval of the Mean Salaries
...
mean_conf_country_tech <- ask_a_manager_2021_cleaned%>%
  ...
  summarise(mean_salary = mean(salary_usd, na.rm=TRUE),
            ...
            salary_upper = mean_salary + t_critical*se_salary)
...
According to the graph, USA and Australia are leading the tech industry in the world in terms of salaries. Is there an actual difference in the mean salaries for tech industry in USA and Australia? In order to proceed, we will conduct hypothesis testing.
...
According to the hypothesis testing, we cannot reject the null hypothesis that there is no difference between the mean salaries for tech employees in USA and Australia.

However there is a clear difference in median salaries between the countries where the USA is a clear leader.
...
## Salary and Gender relationship in Tech Industry

Let us analyse the tech industry further. The countries where the analyses should be conducted are the United States, Canada and the UK  figure \@ref(fig:num-res-per-country) which shows USA, Canda and UK have the highest samples within the dataset. Referring to the figure \@ref(fig:gender-industry-country) we can see that the Tech industry has the most balanced gender data across all countries in the dataset.

Based on this knowledge let us compare the mean and median salaries of man and women within the industry.

### Mean Salary in tech Man and Woman t-test
```{r}
...
graph <- ggplot(median_ci_median_tech_gender, aes(colour = gender)) +
  ...
  scale_x_continuous(labels = function(median) sprintf("%s$",format(median, big.mark = ",")))
graph
```
...
## Salary and Gender Relationship in other industries
...
ask_a_manager_2021_lm <- ask_a_manager_2021_cleaned %>%
 ...
 ask_lm <- lm(salary_usd ~ gender*race + state + years_of_experience_in_field + highest_level_of_education_completed + overall_years_of_professional_experience,
...
Let's look at the health care industry in US. It is in the top 5 popular industries in the US and it is very different from tech. So it may be interesting to see median salary in this industry.
...
How about Accounting, Banking and Finance?
...
The regression isn't so great since almost all of the variables are categorical, so we want to conclude more hypothesis testings to produce more insights about the data.
